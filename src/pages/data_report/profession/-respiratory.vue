...
export default {
  ...
  watch: {
    ...
    handler(){
      // this.$forceUpdate()
      const that = this
      api.respiratory('listReport',func.getyearmonth(this.searchModel)).then(res => {
        if(res.data.data){
  ...
  mounted() {
    const that = this
    api
      .respiratory('listReport', func.getyearmonth(this.searchModel))
      .then((res) => {
        if (res.data.data) {
          that.tableData = res.data.data
        }
