...
export default {
  ...
  watch: {
    ...
    handler(){
      // this.$forceUpdate()
      const that = this
      api.infection('listReport',func.getyearmonth(this.searchModel)).then(res => {
        if(res.data.data){
  ...
  mounted() {
    const that = this
    api
      .infection('listReport', func.getyearmonth(this.searchModel))
      .then((res) => {
        if (res.data.data) {
          that.tableData = res.data.data
        }
  ...
  methods: {
    ...
    async handletablerow(e) {
      ...
      await api
        .infection('listHistogramPro', opt3)
        .then((res) => {
          if (res.data.data) {
            document.getElementById('chartC').innerHTML = ''
            rankchart('chartC', res.data.data, '例数')
