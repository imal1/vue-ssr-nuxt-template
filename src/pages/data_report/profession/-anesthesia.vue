<template>
	<el-row :gutter="20" style="margin: 0;max-height: calc(100vh - 120px);overflow-y: scroll">
		<el-col :span="12">
			<div class="ml-8px text-lg">麻醉专业医疗质量控制指标
				<el-button type="primary" style="float: right;margin-right: 20px" size="mini" plain @click="leadOut">导出</el-button>
			</div>
			<el-table v-if="tableData.length>0" id="printObj" :data="tableData" height="calc(100vh - 150px)" stripe highlight-current-row @row-click="handletablerow">
				<el-table-column prop="name" label="指标名称"  width="55%" cell-style="text-decoration: underline"></el-table-column>
				<el-table-column prop="currentVal" label="本期值"  width="15%"></el-table-column>
				<el-table-column prop="periodVal" label="同期值"  width="15%"></el-table-column>
        		<el-table-column prop="compareVal" label="同比"  width="15%"></el-table-column>
			</el-table>
	...
	export default({
		...
		watch: {
			...
			handler(){
				// this.$forceUpdate()
				const that = this
				api.anesthesia('listReport',func.getyearmonth(this.searchModel)).then(res => {
					if(res.data.data){
	    ...
	    methods: {
	    	...
	    	leadOut(){
				if(this.tableData.length>0){
		        	this.$confirm('是否确定导出文件?', '提示', {}).then(() => {
		        		func.printTable('麻醉专业医疗质量控制指标',document.querySelector("#printObj"))
		        	})
				}else{
			...
			async handletablerow(e){
				...
				await api.anesthesia('listTrendRate',opt).then(res => {
					if(res.data.data){
						document.getElementById('chartB').innerHTML = ''
				...
				await api.anesthesia('listTrend',opt2).then(res => {
					if(res.data.data){
